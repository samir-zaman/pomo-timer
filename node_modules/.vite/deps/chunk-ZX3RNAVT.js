...
function axis(orient, scale2) {
  ...
  function axis2(context) {
    ...
    text.attr(x3, k2 * spacing).text(format2);
...
var Path = class {
  ...
  arcTo(x12, y12, x22, y22, r) {
    ...
    if (this._x1 === null) {
      ...
      let x20 = x22 - x05, y20 = y22 - y05, l21_2 = x21 * x21 + y21 * y21, l20_2 = x20 * x20 + y20 * y20, l21 = Math.sqrt(l21_2), l01 = Math.sqrt(l01_2), l = r * Math.tan((pi - Math.acos((l21_2 + l01_2 - l20_2) / (2 * l21 * l01))) / 2), t01 = l / l01, t21 = l / l21;
  ...
  arc(x3, y3, r, a0, a1, ccw) {
    ...
    if (r < 0) throw new Error(`negative radius: ${r}`);
    let dx = r * Math.cos(a0), dy = r * Math.sin(a0), x05 = x3 + dx, y05 = y3 + dy, cw = 1 ^ ccw, da2 = ccw ? a0 - a1 : a1 - a0;
    if (this._x1 === null) {
...
function clipRectangle(x05, y05, x12, y12) {
  ...
  function corner(p, direction) {
    return abs(p[0] - x05) < epsilon4 ? direction > 0 ? 0 : 3 : abs(p[0] - x12) < epsilon4 ? direction > 0 ? 2 : 1 : abs(p[1] - y05) < epsilon4 ? direction > 0 ? 1 : 0 : direction > 0 ? 3 : 2;
  }
...
function longitude(point6) {
  return abs(point6[0]) <= pi2 ? point6[0] : sign(point6[0]) * ((abs(point6[0]) + pi2) % tau2 - pi2);
}
function polygonContains_default(polygon, point6) {
  var lambda = longitude(point6), phi2 = point6[1], sinPhi = sin(phi2), normal = [sin(lambda), -cos(lambda), 0], angle = 0, winding = 0;
  var sum4 = new Adder();
  if (sinPhi === 1) phi2 = halfPi + epsilon4;
  else if (sinPhi === -1) phi2 = -halfPi - epsilon4;
  for (var i = 0, n = polygon.length; i < n; ++i) {
  ...
  return (angle < -epsilon4 || angle < epsilon4 && sum4 < -epsilon22) ^ winding & 1;
}
  ...
  return {
    ...
    point: function(lambda1, phi1) {
      ...
      if (abs(delta - pi2) < epsilon4) {
        ...
        if (abs(lambda0 - sign0) < epsilon4) lambda0 -= sign0 * epsilon4;
        if (abs(lambda1 - sign1) < epsilon4) lambda1 -= sign1 * epsilon4;
...
function clipAntimeridianIntersect(lambda0, phi0, lambda1, phi1) {
  ...
  return abs(sinLambda0Lambda1) > epsilon4 ? atan((sin(phi0) * (cosPhi1 = cos(phi1)) * sin(lambda1) - sin(phi1) * (cosPhi0 = cos(phi0)) * sin(lambda0)) / (cosPhi0 * cosPhi1 * sinLambda0Lambda1)) : (phi0 + phi1) / 2;
}
...
function rotationIdentity(lambda, phi2) {
  if (abs(lambda) > pi2) lambda -= Math.round(lambda / tau2) * tau2;
  return [lambda, phi2];
}
...
function rotationPhiGamma(deltaPhi, deltaGamma) {
  ...
  function rotation(lambda, phi2) {
    var cosPhi = cos(phi2), x3 = cos(lambda) * cosPhi, y3 = sin(lambda) * cosPhi, z = sin(phi2), k2 = z * cosDeltaPhi + x3 * sinDeltaPhi;
    return [
  ...
  rotation.invert = function(lambda, phi2) {
    var cosPhi = cos(phi2), x3 = cos(lambda) * cosPhi, y3 = sin(lambda) * cosPhi, z = sin(phi2), k2 = z * cosDeltaGamma - y3 * sinDeltaGamma;
    return [
...
function circleRadius(cosRadius, point6) {
  ...
  return ((-point6[2] < 0 ? -radius : radius) + tau2 - epsilon4) % tau2;
}
...
function circle_default(radius) {
  ...
  function clipLine(stream) {
    ...
    return {
      ...
      point: function(lambda, phi2) {
        var point1 = [lambda, phi2], point22, v2 = visible(lambda, phi2), c4 = smallRadius ? v2 ? 0 : code(lambda, phi2) : v2 ? code(lambda + (lambda < 0 ? pi2 : -pi2), phi2) : 0;
  ...
  function intersect(a2, b, two) {
    ...
    var delta2 = lambda1 - lambda0, polar = abs(delta2 - pi2) < epsilon4, meridian = polar || delta2 < epsilon4;
    if (!polar && phi1 < phi0) z = phi0, phi0 = phi1, phi1 = z;
    if (meridian ? polar ? phi0 + phi1 > 0 ^ q[1] < (abs(q[0] - lambda0) < epsilon4 ? phi0 : phi1) : phi0 <= q[1] && q[1] <= phi1 : delta2 > pi2 ^ (lambda0 <= q[0] && q[0] <= lambda1)) {
...
function resample(project, delta2) {
  function resampleLineTo(x05, y05, lambda0, a0, b02, c0, x12, y12, lambda1, a1, b12, c1, depth, stream) {
    var dx = x12 - x05, dy = y12 - y05, d2 = dx * dx + dy * dy;
    if (d2 > 4 * delta2 && depth--) {
      var a2 = a0 + a1, b = b02 + b12, c4 = c0 + c1, m = sqrt(a2 * a2 + b * b + c4 * c4), phi2 = asin(c4 /= m), lambda2 = abs(abs(c4) - 1) < epsilon4 || abs(lambda0 - lambda1) < epsilon4 ? (lambda0 + lambda1) / 2 : atan2(b, a2), p = project(lambda2, phi2), x22 = p[0], y22 = p[1], dx2 = x22 - x05, dy2 = y22 - y05, dz = dy * dx2 - dx * dy2;
      if (dz * dz / d2 > delta2 || abs((dx * dx2 + dy * dy2) / d2 - 0.5) > 0.3 || a0 * a1 + b02 * b12 + c0 * c1 < cosMinDistance) {
        resampleLineTo(x05, y05, lambda0, a0, b02, c0, x22, y22, lambda2, a2 /= m, b /= m, c4, depth, stream);
  ...
  return function(stream) {
    var lambda00, x004, y004, a00, b00, c00, lambda0, x05, y05, a0, b02, c0;
    var resampleStream = {
      point: point6,
      lineStart,
      lineEnd,
    ...
    function linePoint(lambda, phi2) {
      ...
      resampleLineTo(x05, y05, lambda0, a0, b02, c0, x05 = p[0], y05 = p[1], lambda0 = lambda, a0 = c4[0], b02 = c4[1], c0 = c4[2], maxDepth, stream);
    ...
    function ringPoint(lambda, phi2) {
      linePoint(lambda00 = lambda, phi2), x004 = x05, y004 = y05, a00 = a0, b00 = b02, c00 = c0;
      resampleStream.point = linePoint;
    }
    function ringEnd() {
      resampleLineTo(x05, y05, lambda0, a0, b02, c0, x004, y004, lambda00, a00, b00, c00, maxDepth, stream);
...
function projectionMutator(projectAt) {
  var project, k2 = 150, x3 = 480, y3 = 250, lambda = 0, phi2 = 0, deltaLambda = 0, deltaPhi = 0, deltaGamma = 0, rotate, alpha = 0, sx = 1, sy = 1, theta = null, preclip = antimeridian_default, x05 = null, y05, x12, y12, postclip = identity_default3, delta2 = 0.5, projectResample, projectTransform, projectRotateTransform, cache, cacheStream;
...
function conicEqualAreaRaw(y05, y12) {
  ...
  if (abs(n) < epsilon4) return cylindricalEqualAreaRaw(y05);
...
function albersUsa_default() {
  ...
  albersUsa.translate = function(_) {
    ...
    alaskaPoint = alaska.translate([x3 - 0.307 * k2, y3 + 0.201 * k2]).clipExtent([[x3 - 0.425 * k2 + epsilon4, y3 + 0.12 * k2 + epsilon4], [x3 - 0.214 * k2 - epsilon4, y3 + 0.234 * k2 - epsilon4]]).stream(pointStream);
    hawaiiPoint = hawaii.translate([x3 - 0.205 * k2, y3 + 0.212 * k2]).clipExtent([[x3 - 0.214 * k2 + epsilon4, y3 + 0.166 * k2 + epsilon4], [x3 - 0.115 * k2 - epsilon4, y3 + 0.234 * k2 - epsilon4]]).stream(pointStream);
    return reset();
  };
...
function azimuthalRaw(scale2) {
  return function(x3, y3) {
    var cx = cos(x3), cy = cos(y3), k2 = scale2(cx * cy);
...
function mercatorProjection(project) {
  var m = projection(project), center2 = m.center, scale2 = m.scale, translate = m.translate, clipExtent = m.clipExtent, x05 = null, y05, x12, y12;
  m.scale = function(_) {
  ...
  function reclip() {
    var k2 = pi2 * scale2(), t = m(rotation_default(m.rotate()).invert([0, 0]));
...
function conicConformalRaw(y05, y12) {
  ...
  function project(x3, y3) {
    if (f > 0) {
      if (y3 < -halfPi + epsilon4) y3 = -halfPi + epsilon4;
    } else {
      if (y3 > halfPi - epsilon4) y3 = halfPi - epsilon4;
    }
...
function transformer3() {
  ...
  scale2.domain = function(_) {
    return arguments.length ? ([x05, x12, x22] = _, t03 = transform2(x05 = +x05), t13 = transform2(x12 = +x12), t22 = transform2(x22 = +x22), k10 = t03 === t13 ? 0 : 0.5 / (t13 - t03), k21 = t13 === t22 ? 0 : 0.5 / (t22 - t13), s2 = t13 < t03 ? -1 : 1, scale2) : [x05, x12, x22];
  };
  scale2.clamp = function(_) {
...
var asterisk_default = {
  draw(context, size) {
    const r = sqrt3(size + min2(size / 28, 0.75)) * 0.59436;
    const t = r / 2;
    const u4 = t * sqrt32;
    context.moveTo(0, r);
    context.lineTo(0, -r);
...
var plus_default = {
  draw(context, size) {
    const r = sqrt3(size - min2(size / 7, 2)) * 0.87559;
    context.moveTo(-r, 0);
    context.lineTo(r, 0);
    context.moveTo(0, r);
    context.lineTo(0, -r);
...
var times_default = {
  draw(context, size) {
    const r = sqrt3(size - min2(size / 6, 1.7)) * 0.6189;
    context.moveTo(-r, -r);
    context.lineTo(r, r);
    context.moveTo(-r, r);
    context.lineTo(r, -r);
...
var { abs: abs3, max: max3, min: min3 } = Math;
function number1(e) {
  return [+e[0], +e[1]];
}
function number22(e) {
...
function add(tree, x3, y3, d) {
  ...
  while (node.length) {
    ...
    if (bottom2 = y3 >= (ym = (y05 + y12) / 2)) y05 = ym;
    else y12 = ym;
  ...
  function resquarify(parent, x05, y05, x12, y12) {
    if ((rows = parent._squarify) && rows.ratio === ratio) {
      ...
      while (++j < m) {
        ...
        if (row.dice) dice_default(row, x05, y05, x12, value ? y05 += (y12 - y05) * row.value / value : y12);
        else slice_default(row, x05, y05, value ? x05 += (x12 - x05) * row.value / value : x12, y12);
        value -= row.value;
      }
    } else {
...
//# sourceMappingURL=chunk-ZX3RNAVT.js.map
